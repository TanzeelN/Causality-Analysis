...
class fNIRSSignalGenerator(channelLocationMap):
	...
	def __init__(self, nSamples=1, id=1, description='ChannelLocationMap0001', nChannels=1, nOptodes=1,
		...
		self.__samplingRate = 10500/210 #[Hz]
	...
	def double_gamma_function(self, timestamps = np.arange(25, dtype=float), \
		...
		scipy.io.savemat('HRF.mat', {'HRF':HRF})
	...
	def addGaussianNoise(self, channelsList=list(), initSample=0, endSample=-1):
		...
		scipy.io.savemat('time.mat', {'time':timestamps})
	...
	def execute(self):
		...
		boxCarList_OnsetDurations = [(30, 30),(90,30),(150, 30)]
		...
		#enableHHbBlocks[2] = 1  # block 3 is disabled for the HHb signal in all the channels
		...
		self.addStimulusResult(channelsList, boxCarList,
							   ...
							   enableHbO2Blocks=enableHbO2Blocks,
							   enableHHbBlocks=enableHHbBlocks)
		scipy.io.savemat('fnirsdata.mat', {'hbo':self.data[:, :, self.HBO2],'hbr':self.data[:, :, self.HHB]})
		...
		self.addBreathingRateNoise(channelsList, initSample=0, endSample=-1, \
								   frequencyResolutionStep = 0.01)
		scipy.io.savemat('breathingdata.mat', {'hbo':self.data[:, :, self.HBO2],'hbr':self.data[:, :, self.HHB]})
		...
		self.addHeartRateNoise(channelsList, initSample=0, endSample=-1, \
								   frequencyResolutionStep = 0.01)
		scipy.io.savemat('fnirsheartbreathing.mat', {'hbo':self.data[:, :, self.HBO2],'hbr':self.data[:, :, self.HHB]})
		...
		self.addVasomotionNoise(channelsList, initSample=0, endSample=-1, \
								   frequencyResolutionStep = 0.01)
		scipy.io.savemat('vasomotionbreathingheartrate.mat', {'hbo':self.data[:, :, self.HBO2],'hbr':self.data[:, :, self.HHB]})
		...
		scipy.io.savemat('gaussianontop.mat', {'hbo':self.data[:, :, self.HBO2],'hbr':self.data[:, :, self.HHB]})
...
def main():
	...
	sg = fNIRSSignalGenerator(nSamples = 10500, id = newId, description = newDescription,
