...
cfgTEP = [];
%If all channels are being analysed utilised use line below
%cfgTEP.channel = data.label;
%Otherwise, if Specific Channels are being compared
...
cfgTEP.predicttimemax_u    = 25;	  % maximum u to be scanned
cfgTEP.predicttimestepsize = 1; %Information Transfer delay

cfgTEP.TEcalctype = 'VW_ds'; %TE estimator
% ACT estimation and constraints on allowed ACT(autocorelation time)
%Reccommended as high as possible depending on dataset size and
%computational time
cfgTEP.maxlag      = 1000;  % max. lag for the calculation of the ACT
cfgTEP.actthrvalue = 200;   % threshold for ACT
%Manually Change depending on dataset
cfgTEP.minnrtrials = 2;    % minimum acceptable number of trials
...
cfgTEP.ragtaurange    = [0 3]; % tau range to be used
cfgTEP.ragtausteps    = 25;         % steps for ragwitz tau
cfgTEP.repPred        = 800;       % no. local prediction/points used for the Ragwitz criterion
